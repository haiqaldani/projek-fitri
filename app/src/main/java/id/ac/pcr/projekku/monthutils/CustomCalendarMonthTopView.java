...
import id.ac.pcr.projekku.event.MessageEvent;
import id.ac.pcr.projekku.model.DayModel;

public class CustomCalendarMonthTopView extends View {
   ...
   private int numberofrow ;
            ...
            if (event.getX()==downx&&event.getY()==downy){
                ...
                if (cell >= firstday){
                    ...
                    if (pos<dayModels.size()){
                        ...
                        Kalender.lastdate = new LocalDate(year, month, dayModels.get(pos).getDay());
                        Kalender kalender= (Kalender) mContext;
                        if (kalender.mNestedView.getVisibility()==VISIBLE) EventBus.getDefault().post(new MessageEvent(new LocalDate(year, month, dayModels.get(pos).getDay())));
                        if (kalender.weekviewcontainer.getVisibility()==VISIBLE){
                            Calendar todaydate=Calendar.getInstance();
                            todaydate.set(Calendar.DAY_OF_MONTH,Kalender.lastdate.getDayOfMonth());
                            todaydate.set(Calendar.MONTH,Kalender.lastdate.getMonthOfYear()-1);
                            todaydate.set(Calendar.YEAR,Kalender.lastdate.getYear());
                            kalender.mWeekView.goToDate(todaydate);
                        }
                        invalidate();
                    }
                }
    ...
    public CustomCalendarMonthTopView(Context context) {
        this(context, null);
    }

    public CustomCalendarMonthTopView(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CustomCalendarMonthTopView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = context.getTheme().obtainStyledAttributes(attrs, R.styleable.CustomCalendarMonthView, 0, 0);
        try {
            dayHeight = a.getDimensionPixelSize(R.styleable.CustomCalendarMonthView_dayHeight, 200);
            daytextsize = a.getDimensionPixelSize(R.styleable.CustomCalendarMonthView_daytextsize, 12);
            datetextsize = a.getDimensionPixelSize(R.styleable.CustomCalendarMonthView_datetextsize, 14);
            eventtextsize = a.getDimensionPixelSize(R.styleable.CustomCalendarMonthView_eventtextsize, 11);

            daytextcolor = a.getColor(R.styleable.CustomCalendarMonthView_daytextcolor, Color.GRAY);
            datetextcolor = a.getColor(R.styleable.CustomCalendarMonthView_datetextcolor, Color.GRAY);


            datemargintop=a.getDimensionPixelSize(R.styleable.CustomCalendarMonthView_datemargintop,25);
            linecolor = a.getColor(R.styleable.CustomCalendarMonthView_linecolor, Color.GRAY);
            linewidth = a.getDimensionPixelSize(R.styleable.CustomCalendarMonthView_linewidth,2);
        ...
        for (int i=0;i<numberofrow;i++) {
            for (int j = 0; j < 7 ; j++) {
                ...
                if (position>=firstday){
                    ...
                    boolean selected = dayModel.getDay() == Kalender.lastdate.getDayOfMonth() && dayModel.getMonth() == Kalender.lastdate.getMonthOfYear() && dayModel.getYear() == Kalender.lastdate.getYear() ? true : false;
