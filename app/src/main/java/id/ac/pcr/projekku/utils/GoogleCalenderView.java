package id.ac.pcr.projekku.utils;
...
import id.ac.pcr.projekku.event.AddEvent;
...
import id.ac.pcr.projekku.monthutils.CustomCalendarMonthTopView;
import id.ac.pcr.projekku.monthutils.MonthChangeListner;
...
//       });
        viewPager.registerOnPageChangeCallback(new ViewPager2.OnPageChangeCallback() {
            ...
            public void onPageSelected(int position) {
                ...
                Kalender kalender = (Kalender) context;
                currentmonth = position;

                if (!kalender.isAppBarClosed()) {

                    adjustheight();
                    if (kalender.mNestedView.getVisibility()==VISIBLE)EventBus.getDefault().post(new MessageEvent(new LocalDate(myPagerAdapter.monthModels.get(position).getYear(), myPagerAdapter.monthModels.get(position).getMonth(), 1)));
                    else {
                        Kalender.lastdate=new LocalDate(myPagerAdapter.monthModels.get(position).getYear(), myPagerAdapter.monthModels.get(position).getMonth(), 1);
                    }
...
//               }
    ...
    public void updategrid() {
        ...
        if (myPagerAdapter != null) {
            ...
            if (monthViewHolder != null && monthViewHolder.cCalendarMonthTopView != null ) {

                monthViewHolder.cCalendarMonthTopView.requestLayout();
                monthViewHolder.cCalendarMonthTopView.invalidate();
...
//    }
    ...
    class MonthPagerAdapter extends RecyclerView.Adapter<MonthPagerAdapter.MonthViewHolder> {
        ...
        public void onBindViewHolder(@NonNull MonthViewHolder holder, int position) {
            ...
            holder.cCalendarMonthTopView.initdata(monthtemp.getDayModelArrayList(), monthtemp.getFirstday(), monthtemp.getMonth(), monthtemp.getYear());
...
//            dayadapter.notifyDataSetChanged();
        ...
        class MonthViewHolder extends RecyclerView.ViewHolder {
           ...
           // RecyclerView gridview;
            CustomCalendarMonthTopView cCalendarMonthTopView;
            MonthViewHolder(View itemView) {
                super(itemView);
                cCalendarMonthTopView = itemView.findViewById(R.id.ccalendarmonthview);
                ...
                itemView.setOnClickListener(new OnClickListener() {
                    ...
                    public void onClick(View view) {
                        if (getAdapterPosition() >= firstday) {
                            ...
                            Kalender.lastdate = new LocalDate(year, month, dayModels.get(getAdapterPosition() - firstday).getDay());
                            Kalender kalender= (Kalender) context;
                            if (kalender.mNestedView.getVisibility()==VISIBLE)EventBus.getDefault().post(new MessageEvent(new LocalDate(year, month, dayModels.get(getAdapterPosition() - firstday).getDay())));
