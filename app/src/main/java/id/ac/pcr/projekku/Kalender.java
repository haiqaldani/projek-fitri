...
import id.ac.pcr.projekku.monthutils.MonthChange;
import id.ac.pcr.projekku.monthutils.MonthChangeListner;
import id.ac.pcr.projekku.monthutils.MonthFragment;
import id.ac.pcr.projekku.utils.CustomTypefaceSpan;
import id.ac.pcr.projekku.utils.GoogleCalenderView;
import id.ac.pcr.projekku.utils.MyAppBarBehavior;
import id.ac.pcr.projekku.utils.MyRecyclerView;
import id.ac.pcr.projekku.utils.Utility;
import id.ac.pcr.projekku.weekview.DateTimeInterpreter;
import id.ac.pcr.projekku.weekview.MonthLoader;
import id.ac.pcr.projekku.weekview.WeekView;
import id.ac.pcr.projekku.weekview.WeekViewEvent;
import id.ac.pcr.stickyheadersrecyclerview.StickyRecyclerHeadersAdapter;
import id.ac.pcr.stickyheadersrecyclerview.StickyRecyclerHeadersDecoration;

public class Kalender extends AppCompatActivity implements MyRecyclerView.AppBarTracking, WeekView.EventClickListener, MonthLoader.MonthChangeListener, WeekView.EventLongPressListener, WeekView.EmptyViewLongPressListener, WeekView.ScrollListener {

    private static final String TAG = "Kalender";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        monthviewpager.setCurrentItem(calendarView.calculateCurrentMonth(Kalender.lastdate), true);
        ...
        monthviewpager.addOnPageChangeListener(new ViewPager.OnPageChangeListener() {
            ...
            public void onPageSelected(int i) {
                ...
                if (isAppBarClosed()) {
                    ...
                    Kalender.lastdate = new LocalDate(monthModel.getYear(), monthModel.getMonth(), 1);
                }
            }
    ...
    public void onBackPressed() {
        if (closebtn.getVisibility() == View.VISIBLE) {
            ...
            monthviewpager.setCurrentItem(calendarView.calculateCurrentMonth(Kalender.lastdate), false);
...
//            mArrowImageView.setVisibility(View.INVISIBLE);
    ...
    public void selectdateFromMonthPager(int year, int month, int day) {
        Kalender.lastdate = new LocalDate(year, month, day);
        LocalDate localDate = new LocalDate();
        String yearstr = Kalender.lastdate.getYear() == localDate.getYear() ? "" : Kalender.lastdate.getYear() + "";
        monthname.setText(Kalender.lastdate.toString("MMMM") + " " + yearstr);
        calendarView.setCurrentmonth(Kalender.lastdate);
        calendarView.adjustheight();
        mIsExpanded = false;
        mAppBar.setExpanded(false, false);
